...
def _find_tile_dirs(root_dir, split_ratio, tile_name):
    """
...
def merge_tiles(root_dir, split_ratio, tile_name, output_dir):
    """
    Merge tiles (found in subfolders) into a single PIL image in memory.
    Returns (merged_pil_image, (width, height), pil_fmt, ext).
    NOTE: This no longer saves the full-size merged image to disk.
    """
    os.makedirs(output_dir, exist_ok=True)

    mapping = _find_tile_dirs(root_dir, split_ratio, tile_name)
    ...
    # Create the full canvas (in memory)
    full = Image.new(common_mode, (total_width, total_height))

    # Paste all tiles
    for i in tqdm(range(split_ratio), desc="Merging rows"):
        for j in tqdm(range(split_ratio), desc="Merging cols", leave=False):
            tile_path = mapping[(i, j)]
            with Image.open(tile_path) as im:
                im = im.convert(common_mode)
                full.paste(im, (x_offsets[j], y_offsets[i]))

    print("Merged full image created in memory (not saved to disk).")
    return full, (total_width, total_height), pil_fmt, ext

def save_downscaled(merged_img, original_size, output_dir, base, factor, pil_fmt, ext):
    """
    Save a downscaled (1/factor) version of the in-memory merged image, preserving suffix.
    """
    os.makedirs(output_dir, exist_ok=True)
    w, h = original_size
    dw = max(1, w // factor)
    ...
    target_mode = "RGB" if pil_fmt == "JPEG" else ("RGBA" if "A" in merged_img.mode else "RGB")
    im = merged_img
...
def main():
    parser = argparse.ArgumentParser(
        description="Merge tiles from subfolders named like '..._<i>_<j>' and save ONLY a downscaled image."
    )
    ...
    parser.add_argument("--split_ratio", type=int, default = 3,
                        help="Splits per axis (e.g., 3 means a 3x3 grid)")
    ...
    parser.add_argument("--output_dir", default = "/media/zzd/hrhsr/backup/Documents/img_seg/River/data/data_2023_mavic_p4/p4/test_data_1/mavic3_pro_10_2_2023_transparent_mosaic_group1/step1_result/",
                        help="Folder to save the downscaled image")
    ...
    merged_img, size_wh, pil_fmt, ext = merge_tiles(
        args.root_dir, args.split_ratio, args.tile_name, args.output_dir
    )

    base = os.path.basename(os.path.normpath(args.root_dir))
    save_downscaled(
        merged_img, size_wh, args.output_dir, base, args.downscale_factor, pil_fmt, ext
    )
